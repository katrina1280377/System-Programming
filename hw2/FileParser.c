...
char* ReadFileToString(const char* file_name, int* size) {
  ...
  while (left_to_read > 0) {
    num_read = read(fd, buf + (file_size - left_to_read), left_to_read);
    if (num_read == -1) {
...
static void InsertContent(HashTable* tab, char* content) {
  ...
  while (*cur_ptr != '\0') {
    if (isalpha(*cur_ptr)) {
      ...
      if (word_start == NULL) {
        word_start = cur_ptr;
        pos = (int)(cur_ptr - content);
      }
      *cur_ptr = tolower(*cur_ptr);
    } else if (word_start != NULL) {
        char original_char = *cur_ptr;
        *cur_ptr = '\0';
        // Add the word to the hash table
        AddWordPosition(tab, word_start, pos);
        *cur_ptr = original_char;
        // Reset word_start to NULL to indicate that we're not inside a word
        word_start = NULL;
    }
    cur_ptr++;  // Move to the next char
  }

  // If the content ends with a word, process it
  if (word_start != NULL) {
    AddWordPosition(tab, word_start, pos);
  }
}
...
static void AddWordPosition(HashTable* tab, char* word,
  ...
  if (HashTable_Find(tab, hash_key, &kv)) {
    ...
    LinkedList* new_list = LinkedList_Allocate();
    LinkedList_Append(new_list, (LLPayload_t) (int64_t) pos);
    wp->positions = new_list;
    kv.key = hash_key;
    kv.value = wp;
    HTKeyValue_t oldkeyvalue;
    HashTable_Insert(tab, kv, &oldkeyvalue);
  }
}
