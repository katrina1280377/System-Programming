...
void MemIndex_AddPostingList(MemIndex* index, char* word, DocID_t doc_id,
  ...
  if (!HashTable_Find(index, key, &mi_kv)) {
    ...
    // Allocate a new hashtable for the WordPostings' docID->postings mapping.
    wp->postings = MemIndex_Allocate();
    if (wp->postings == NULL) {
      free(wp->word);
      free(wp);
      // Handle memory allocation failure here, if needed.
    ...
    // Before inserting, ensure that the same key is not already present.
    Verify333(!HashTable_Find(index, key, &mi_kv));
...
LinkedList* MemIndex_Search(MemIndex* index, char* query[], int query_len) {
  ...
  char* first_query = query[0];
  // Calculate the hash key for the first query word.
  key = FNVHash64((unsigned char*)first_query, strlen(first_query));

  // Look up the query word in the inverted index.
  if (!HashTable_Find(index, key, &kv)) {
    return NULL;
  }

  // Get the WordPostings for the first query word.
  wp = kv.value;
  ...
  HTIterator* ht_iter = HTIterator_Allocate(wp->postings);
  if (ht_iter == NULL) {
    LinkedList_Free(ret_list, (LLPayloadFreeFnPtr)free);
    return NULL;
  }

  int hash_size = HashTable_NumElements(wp->postings);
  for (int i = 0; i < hash_size; i++) {
    if (HTIterator_Get(ht_iter, &kv) != 0) {
      ...
      HTIterator_Next(ht_iter);
    }
  }
  HTIterator_Free(ht_iter);
  ...
  for (i = 1; i < query_len; i++) {
    ...
    if (!HashTable_Find(index, key, &kv)) {
      // No matches for the current query word, return NULL.
      LinkedList_Free(ret_list, (LLPayloadFreeFnPtr)free);
      return NULL;
    }
    ...
    for (j = 0; j < num_docs; j++) {
      ...
      LLIterator_Get(ll_it, (LLPayload_t*)&payload);
