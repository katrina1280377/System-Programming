...
DocID_t DocTable_Add(DocTable* table, char* doc_name) {
  ...
  HTKey_t hashkey = FNVHash64((unsigned char*) doc_name, strlen(doc_name));
  res = HashTable_Find(table->name_to_id, hashkey, &kv);
  if (res == 1) {
  ...
  strncpy(doc_copy, doc_name, 1+strlen(doc_name));
