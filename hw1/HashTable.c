...
bool HashTable_Insert(HashTable *table,
  ...
  HTKeyValue_t*  chain_keyvalue;

  // To see if the key in newkeyvalue is already contained in one of the nodes.
  while (LLIterator_IsValid(iter)) {
    LLIterator_Get(iter, (LLPayload_t) &chain_keyvalue);
    if (chain_keyvalue->key == newkeyvalue.key) {
      if (oldkeyvalue != NULL) {
        // Copy the old key-value pair into the provided oldkeyvalue pointer.
        oldkeyvalue->key = chain_keyvalue->key;
        oldkeyvalue->value = chain_keyvalue->value;
      }
      // Replace the existing key-value pair with the new one.
      chain_keyvalue->key = newkeyvalue.key;
      chain_keyvalue->value = newkeyvalue.value;
      LLIterator_Free(iter);
      return true;
    }
    LLIterator_Next(iter);
  }
  // Insert new key if it is not found in the Hash table
  HTKeyValue_t *new_keyvalue = (HTKeyValue_t *) malloc(sizeof(HTKeyValue_t));
  new_keyvalue->key = newkeyvalue.key;
  new_keyvalue->value = newkeyvalue.value;
  LinkedList_Push(chain, (LLPayload_t) new_keyvalue);
...
bool HashTable_Find(HashTable *table,
  ...
  while (LLIterator_IsValid(iter)) {
    HTKeyValue_t*  chain_keyvalue;
    LLIterator_Get(iter, (LLPayload_t) &chain_keyvalue);
    if (chain_keyvalue->key == key) {
      if (keyvalue != NULL) {
          keyvalue->key = chain_keyvalue->key;
          keyvalue->value = chain_keyvalue->value;
     }
      free(iter);
      return true;
    }
...
bool HashTable_Remove(HashTable *table,
  ...
  while (LLIterator_IsValid(iter)) {
    LLIterator_Get(iter, (LLPayload_t) &chain_keyvalue);
    // Remove the element from the linked list if the key is found
    if (chain_keyvalue->key == key) {
      keyvalue->key = chain_keyvalue->key;
      keyvalue->value = chain_keyvalue->value;
...
bool HTIterator_IsValid(HTIterator *iter) {
  ...
  // Check if the iter is pointing to a valid entry or iterator is at the end.
  if (iter->ht->num_elements == 0 || !LLIterator_IsValid(iter->bucket_it)) {
    return false;
  }
...
bool HTIterator_Next(HTIterator *iter) {
  ...
  // If reached the end of the current linked list
...
bool HTIterator_Get(HTIterator *iter, HTKeyValue_t *keyvalue) {
  ...
  // Cast the keyvalue to HTKeyValue_t* and copy its contents
  HTKeyValue_t *new_keyvalue;
  LLIterator_Get(iter->bucket_it, (LLPayload_t) &new_keyvalue);
  keyvalue->key = new_keyvalue->key;
  keyvalue->value = new_keyvalue->value;
