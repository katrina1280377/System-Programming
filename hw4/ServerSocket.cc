...
bool ServerSocket::BindAndListen(int ai_family, int* const listen_fd) {
  ...
  // STEP 1, refer to server_bind_listen.cc in lecture
  ...
  // Obtain a string representation of the port number in order to pass
  // it to the getaddrinfo() function. The getaddrinfo() function retrieves
  // a list of address structures through the output parameter called "result".
  struct addrinfo* result;
  string port = std::to_string(this->port_);
  ...
  for (struct addrinfo* rp = result; rp != nullptr; rp = rp->ai_next) {
    ...
    // to make the port we bind to available again as soon as we
    // exit.
    int optval = 1;
...
bool ServerSocket::Accept(int* const accepted_fd,
  ...
  // STEP 2: check from server_accept_rw_close.cc
  int client_fd;
  ...
  *accepted_fd = client_fd;
  ...
  if (caddr.ss_family == AF_INET) {
    // Get out the IPV4 address and port
    ...
    *client_port = htons(in4->sin_port);
    ...
    // Get out the IPV6 address and port
  ...
  char client[kBufferLength];
  client[0] = '\0';  // Initialize the client array
  if (getnameinfo(reinterpret_cast<struct sockaddr*>(&caddr),
    caddr_len, client, kBufferLength, NULL, 0, 0) != 0) {
    snprintf(client, kBufferLength, "[reverse DNS failed]");
  }
  *client_dns_name = string(client);

  // Get the server's addr and dns name
  char server[kBufferLength];
  server[0] = '\0';  // Initialize the server array
  if (sock_family_ == AF_INET) {
    ...
    getnameinfo(reinterpret_cast<struct sockaddr*>(&srvr), srvrlen, server,
                kBufferLength, nullptr, 0, 0);

    *server_addr = string(addrbuf);
    *server_dns_name = string(server);
  } else {
    // IPV6
