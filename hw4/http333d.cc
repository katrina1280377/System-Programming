...
static void GetPortAndPath(int argc,
  ...
  *path = string(argv[2]);
  ...
  // Parse the list of index files
  for (int i = 3; i < argc; i++) {
    string index = argv[i];
    struct stat index_stat;

    if (stat(index.c_str(), &index_stat) != 0 || !S_ISREG(index_stat.st_mode)
        || access(index.c_str(), R_OK) != 0) {
      cerr << "Not a readable directory: " << index  << endl;
      Usage(argv[0]);
    }
    // Add indices to indices list if they are readable
    indices->push_back(index);
  }

  // At least one index file is provided
  if (indices->empty()) {
    cerr << "Please provide at least 1 index file." << endl;
    Usage(argv[0]);
  }
}
