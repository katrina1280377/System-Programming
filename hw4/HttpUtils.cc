...
bool IsPathSafe(const string& root_dir, const string& test_file) {
  ...
  // Fetch the complete file paths for root_dir and test_file.
  if (realpath(root_dir.c_str(), root_dir_path) == nullptr ||
      realpath(test_file.c_str(), test_file_path) == nullptr) {
    return false;
  }

  // Check the path of root_dir is equal to or longer than the path of
  // test_file.
  size_t root_dir_len = strlen(root_dir_path);
  if (root_dir_len > 0 && root_dir_path[root_dir_len - 1] != '/') {
    // Add a trailing slash
    root_dir_path[root_dir_len] = '/';
    root_dir_path[root_dir_len + 1] = '\0';
  }
  if (root_dir_len >= strlen(test_file_path) ||
      strncmp(root_dir_path, test_file_path, strlen(root_dir_path)) != 0) {
    return false;
  }
