...
static int WriteHashTable(FILE* f, IndexFileOffset_t offset, HashTable* ht,
  ...
  for (int i = 0; i < ht->num_buckets; i++) {
    // STEP 4.
    // Write the bucket record into the file.
    int num_elts = LinkedList_NumElements(ht->buckets[i]);
    int br_bytes = WriteHTBucketRecord(f, record_pos, num_elts, bucket_pos);

    // If the bucket is not empty, write the bucket contents
    // and update the bucket position
    if (num_elts != 0) {
      int ht_bytes = WriteHTBucket(f, bucket_pos, ht->buckets[i], fn);
      bucket_pos += ht_bytes;
    }
    // Update the record position for the next iteration.
    record_pos += br_bytes;
  }
...
static int WriteHTBucket(FILE* f, IndexFileOffset_t offset, LinkedList* li,
  ...
  for (int i = 0; i < num_elts; i++) {
    ...
    int elt_size = fn(f, element_pos, kv);
    if (elt_size < 0) {
      LLIterator_Free(it);
      return kFailedWrite;
    }
    ...
    element_pos += elt_size;  // element size
    LLIterator_Next(it);
  }
  LLIterator_Free(it);
...
static int WriteDocIDToPositionListFn(FILE* f,
  ...
  for (int i = 0; i < num_positions; i++) {
    ...
    DocIDElementPosition dc_position;
    LLPayload_t payload;
    LLIterator_Get(it, &payload);
    // STEP 14.
    // Truncate to 32 bits, then convert it to network order and write it out.
    dc_position.position = *reinterpret_cast<uint32_t*>(&payload);
    dc_position.ToDiskFormat();
    if (fwrite(&dc_position, sizeof(uint32_t), 1, f) != 1) {
      LLIterator_Free(it);
      return kFailedWrite;
    }
    total_bytes += sizeof(dc_position);
