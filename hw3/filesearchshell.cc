...
static void PrintResult(const vector<hw3::QueryProcessor::QueryResult>&);
...
int main(int argc, char** argv) {
  ...
  // Instantiate QueryProcessor with the provided index files
  list<string> indexFiles(argv + 1, argv + argc);
  hw3::QueryProcessor query(indexFiles, true);
  while (1) {
    cout << "Enter query:" << endl;
    // Read a line of input
    string userInput;
    if (!getline(cin, userInput) || cin.eof()) {
    ...
    auto result = query.ProcessQuery(query_words);
    PrintResult(result);
  }
...
static void PrintResult(const vector<hw3::QueryProcessor::QueryResult> &res) {
  if (res.empty()) {
    cout << "  [no results]" << endl;
    return;
  }
  for (auto const& em : res) {
    cout << em.document_name << " (" << to_string(em.rank) << ")" << endl;
  }
}
